...
function drawUI(ctx) {
  ...
  if (activePowerups.length > 0) {
    ...
    const inventoryRightEdge =
      INVENTORY_CONFIG.baseX +
      INVENTORY_CONFIG.maxSlots *
        (INVENTORY_CONFIG.buttonSize + INVENTORY_CONFIG.gap) -
      INVENTORY_CONFIG.gap;
    const powerupHudX = inventoryRightEdge + 10; // 10px margin from inventory
    const powerupHudY =
      INVENTORY_CONFIG.baseY + (INVENTORY_CONFIG.buttonSize - 36) / 2; // Vertically centered
    drawPowerupHUD(ctx, activePowerups, powerupHudX, powerupHudY);
    topbarHeight = Math.max(
      topbarHeight,
      INVENTORY_CONFIG.baseY + INVENTORY_CONFIG.buttonSize,
    );
  }
...
function drawGameOver(ctx, canvas, showSocialFeatures = true) {
  ...
  if (rating) {
    statsContainer.addChild(
      flexItem().text(rating, 14).textStyle(ratingColor, 14, "Arial", "bold"),
    );
  }
  ...
  const gameOverLeaderboardBounds = gameOverPanel.getTaggedBounds(
    "gameOverLeaderboardButton",
  );
  if (gameOverLeaderboardBounds)
    setButtonBounds(
      "gameOverLeaderboardButton",
      gameOverLeaderboardBounds.x,
      gameOverLeaderboardBounds.y,
      gameOverLeaderboardBounds.width,
      gameOverLeaderboardBounds.height,
    );
...
function drawStartScreen(ctx, canvas, isPaused = false) {
  ...
  if (isPaused) {
    // 暂停菜单中的重新开始按钮
    const restartFromPauseBounds = panel.getTaggedBounds(
      "restartFromPauseButton",
    );
    if (restartFromPauseBounds)
      setButtonBounds(
        "restartFromPauseButton",
        restartFromPauseBounds.x,
        restartFromPauseBounds.y,
        restartFromPauseBounds.width,
        restartFromPauseBounds.height,
      );
  }
  ...
  if (leaderboardBounds)
    setButtonBounds(
      "leaderboardButton",
      leaderboardBounds.x,
      leaderboardBounds.y,
      leaderboardBounds.width,
      leaderboardBounds.height,
    );
  ...
  if (birdAlbumBounds)
    setButtonBounds(
      ...
      birdAlbumBounds.x,
      birdAlbumBounds.y,
      birdAlbumBounds.width,
      birdAlbumBounds.height,
    );
}
