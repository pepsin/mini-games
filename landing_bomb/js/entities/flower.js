...
// Draw single flower
function drawFlower(ctx, x, y, alive, frameIndex, hasShield = false) {
  const flowerRes = getResource('flower');
  const flowerCoveredRes = getResource('flower_covered');

  if (isResourcesLoaded() && flowerRes) {
    // If shield is active and flower is alive, show covered image
    if (hasShield && alive && flowerCoveredRes && flowerCoveredRes.image) {
      const size = animationLoader.getSize(flowerCoveredRes);
      const anchor = animationLoader.getAnchor(flowerCoveredRes);
      drawImageProportional(ctx, flowerCoveredRes.image, x, y, size.width, anchor.x, anchor.y);
      return;
    }
...
function drawHealthFlowers(ctx, hasShield = false) {
  ...
  for (let i = 0; i < 4; i++) {
    ...
    drawFlower(ctx, pos.x, pos.y, flowerAlive[i], flowerFrameIndices[i], hasShield);
  }
}
